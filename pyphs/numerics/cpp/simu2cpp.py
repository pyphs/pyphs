...
def simu2cpp(simu):
    ...
    with open(os.path.join(path_to_templates, "cpp", "simu_main.template"), "r") as f:
    ...
    with open(os.path.join(path_to_templates, "cpp", "simu_bash.template"), "r") as f:
...
def _str_updateResults(simu, objlabel):
    ...
    for name in simu.data.names[2:]:
        ...
        if dim > 0:
            ...
            updateResults += (
                """
for (unsigned int ind=0; ind<{dim}; ind++)""".format(
                    **temp
                )
                + "{"
                + """
