...
class Fracderec(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        for n in range(nbPoles):
            ...
            N1 = "N" + label + str(n) + "_1"
            N2 = "N" + label + str(n) + "_2"
            self += DissipativeLinear(
                label + "R" + str(n), (N1, Ndeb), inv_coeff=True, coeff=Rn, ctrl="e"
            )

            Qn = diagQ[n]
            self += StorageLinear(
                label + "Q" + str(n), (N2, datum), value=Qn, ctrl="f", inv_coeff=False
            )

            self += Transformer(
                label + "alpha" + str(n), (N1, Nend, N2, datum), alpha=Rn ** -1
            )

    metadata = {
        "title": "Effort-controlled fractional derivator",
        "component": "Fracderec",
        "label": "fracder",
        "dico": "fraccalc",
        "desc": r"Effort-controlled fractional derivator from [1]_ (chap 7):"
        + equation(r"f(s) = g \, s^{alpha}  \, e(s)."),
        ...
        "parameters": [
            ...
            [
                "PolesMinMax",
                "Poles modules in :math:`(10^{min}, 10^{max})`",
                "Hz",
                (-5, 10),
            ],
...
class Fracderfc(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        for n in range(nbPoles):
            ...
            self += DissipativeLinear(
                label + "R" + str(n), (N2, N1), ctrl="f", coeff=Rn, inv_coef=False
            )

            Qn = diagQ[n]
            N3 = "N" + label + str(n) + "_2"
            self += StorageLinear(
                label + "Q" + str(n), (N3, datum), value=Qn, inv_coeff=False, ctrl="e"
            )

            Nend = nodes[1]
            self += Transformer(label + "alpha" + str(n), (N1, N2, N3, datum), alpha=Rn)
            N1 = N2

    metadata = {
        "title": "Flux-controlled fractional derivator",
        "component": "Fracderfc",
        "label": "fracder",
        "dico": "fraccalc",
        "desc": r"Flux-controlled fractional derivator from [1]_ (chap 7):"
        + equation(r"e(s) = g \, s^{alpha}  \, f(s)."),
        "nodesdesc": "Component terminals with positive flux N1->N2.",
        "nodes": ("N1", "N2"),
        "parametersdesc": "Component parameters",
        "parameters": [
...
def fractionalDifferenciatorWeights(
    p,
    alpha,
    NbPoles=20,
    PolesMinMax=(-5, 10),
