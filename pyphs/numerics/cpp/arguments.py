...
def _append_args_accessors_vector(method, files, objlabel):
    ...
    title = linesplit + "\n// Acessors to Arguments, return vector<{0}>".format(
        CONFIG_CPP["float"]
    )
    ...
    for name in method.args_names:
        ...
        dim = len(arg)
        files["h"]["public"] += "\nconst vector<{1}> & {0}_vector() const;".format(
            name, CONFIG_CPP["float"]
        )
        files["cpp"]["public"] += (
        ...
        files["cpp"]["public"] += indent(
            "\nstatic vector<{1}> v = vector<{1}>({0});".format(
                dim, CONFIG_CPP["float"]
            )
        )
...
def _append_args_accessors_matrix(method, files, objlabel):
    ...
    title = linesplit + "\n// Acessors to Arguments, return Matrix<{0}, n, m>".format(
        CONFIG_CPP["float"]
    )
    files["h"]["public"] += title
    files["cpp"]["public"] += title
    for name in method.args_names:
        ...
        if dim0 == 0:
            ...
            dim1 = 1
        files["h"]["public"] += "\nconst {0} & {1}() const;".format(
            matrix_type(dim0, dim1), name
        )
        files["cpp"]["public"] += (
            "\nconst {0} & {1}::{2}() const".format(
                matrix_type(dim0, dim1), objlabel, name
            )
            + " {"
        )
        files["cpp"]["public"] += indent(
...
def _append_args_mutators_matrix(method, files, objlabel):
    ...
    title = linesplit + "\n// Mutators for Arguments, type = Matrix<{0}, n, m>".format(
        CONFIG_CPP["float"]
    )
    files["h"]["public"] += title
    files["cpp"]["public"] += title
    for name in method.args_names:
        ...
        files["cpp"]["public"] += (
            ...
            + "(const Matrix<{2}, {0}, {1}> & m)".format(
                dim0, dim1, CONFIG_CPP["float"]
            )
            + " {"
        )
        for i, symb in enumerate(arg):
...
def _append_args_mutators_vectors(method, files, objlabel):
    ...
    title = linesplit + "\n// Mutators for Arguments, type = vector<{0}>".format(
        CONFIG_CPP["float"]
    )
    files["h"]["public"] += title
    files["cpp"]["public"] += title
    for name in method.args_names:
        arg = getattr(method, name + "_expr")
        files["h"]["public"] += "\nvoid set_{0}(const vector<{1}> &);".format(
            name, CONFIG_CPP["float"]
        )
        files["cpp"]["public"] += (
            "\nvoid {0}::set_{1}(const vector<{2}> & v)".format(
                objlabel, name, CONFIG_CPP["float"]
            )
            + " {"
        )
        for i, symb in enumerate(arg):
...
def _append_args_mutators_elements(method, files, objlabel):
    ...
    title = (
        linesplit
        + "\n// Mutators for a single argument, types are {0} with int index".format(
            CONFIG_CPP["float"]
        )
    )
    files["h"]["public"] += title
    ...
    for name in method.args_names:
        arg = getattr(method, name + "_expr")
        files["h"]["public"] += "\nvoid set_{0}(const {1} &, unsigned int &);".format(
            name, CONFIG_CPP["float"]
        )
        files["cpp"]["public"] += (
            "\nvoid {0}::set_{1}(const {2} & value, unsigned int & index)".format(
                objlabel, name, CONFIG_CPP["float"]
            )
            + " {"
        )
