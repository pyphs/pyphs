...
class Storage(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        assert all(
            h_vals[np.nonzero(x_vals >= 0)] >= 0
        ), "All values h(x) for\
 x>=0 must be non-negative (component {})".format(
            label
        )
 ...
 for x<0 must be negative (component {})".format(
                label
            )
        else:
            assert all(
 ...
 for x<0 must be non-negative (component {})".format(
                label
            )

        assert (
            h_vals[np.nonzero(x_vals == 0)] == 0
        ), "dxh(0) and h(0) must be \
zero (component {})".format(
            label
        )
        ...
        # edge data
        data = {"label": x, "type": "storage", "ctrl": ctrl, "file": path, "link": None}
        N1, N2 = nodes
    ...
    metadata = {
        ...
        "parameters": [
            [
                "file",
                "Path to data file for (x, H) or (x, dxH) values",
                "string",
                "example.txt",
            ],
            [
                "integrate",
                "If True, data is (x, dxH) and integrate to (x, H)",
                "bool",
                False,
            ],
