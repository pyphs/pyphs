...
class Fracintec(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        for n in range(nbPoles):
            ...
            comp = DissipativeLinear(
                "R_" + label + str(n), (Ncomp, Nend), coeff=Rn, ctrl="f"
            )
            self += comp
    ...
    metadata = {
        "title": "Effort-controlled fractional integrator",
        "component": "Fracintec",
        "label": "fracint",
        "dico": "fraccalc",
        "desc": "Effort-controlled fractional integrator from [1]_ (chap 7):"
        + equation(r"f(s) = g \, s^{-beta}  \, e(s)."),
    ...
    def __init__(self, label, nodes, **kwargs):
        ...
        for n in range(nbPoles):
            ...
            self += DissipativeLinear(
                label + "R" + str(n), nodes, coeff=Rn, inv_coeff=True, ctrl="e"
            )

            Qn = diagQ[n]
            self += StorageLinear(
                label + "Q" + str(n), nodes, value=Qn, inv_coeff=False, ctrl="f"
            )
            N = Ncomp

    metadata = {
        "title": "Flux-controlled fractional integrator",
        "component": "Fracintfc",
        "label": "fracint",
        "dico": "fraccalc",
        "desc": r"Flux-controlled fractional integrator from [1]_ (chap 7)"
        + equation(r"e(s) = g \, s^{-beta}  \, f(s)."),
        "nodesdesc": "Component terminals with positive flux N1->N2.",
        "nodes": ("N1", "N2"),
        "parametersdesc": "Component parameters",
        "parameters": [
...
def fractionalIntegratorWeights(
    p,
    beta,
    NbPoles=10,
    PolesMinMax=(-10, 10),
