...
def plot_power_balance_rlc_with_split():
    ...
    dur = 0.01
    u = signalgenerator(which="sin", f0=800.0, tsig=dur, fs=simu.config["fs"])
...
def dataH5File():
    ...
    start = int(simu.data.nt / 10.0)
    stop = int(9 * simu.data.nt / 10.0)
    step = 3
...
def plot_rlc_with_split():
    ...
    # plot u, y
    simu.data.plot(
        [("u", i) for i in range(dims.y())] + [("y", i) for i in range(dims.y())],
        show=False,
    )
    ...
    simu.data.plot(
        [("dtx", i) for i in range(dims.x())] + [("dxH", i) for i in range(dims.x())],
        show=False,
    )
