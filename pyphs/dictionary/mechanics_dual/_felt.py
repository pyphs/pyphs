...
class Felt(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        xnl = symbols("q" + label)
        hnl = sp.Piecewise(
            (0.0, xnl <= 0.0), ((L * F / (B + 1)) * (xnl / L) ** (B + 1), True)
        )
        hnl = hnl.subs(dicpars)
        # edge data
        data = {"label": xnl, "type": "storage", "ctrl": "f", "link": None}
        self.add_edges_from(
            [
                (N1, N3, data),
            ]
        ...
        r = sp.Piecewise((0.0, xnl <= 0.0), ((A * L / B) * (xnl / L) ** (B - 1), True))
        r = r.subs(dicpars)
    ...
    metadata = {
        ...
        "dico": "mechanics_dual",
        "desc": (
            r"Nonlinear felt material used in piano-hammer. The model is that found in [1]_ eq. (11). It includes a nonlinear restoring force and a nonlinear damper as follows:"
            + equation(
                r"e_{total}\left(c, \dot c\right) = e_{elastic}(c) + e_{damper}\left(c, \dot c\right),"
            )
            + "with"
            + equation(r"e_{elastic}(c)  = F \,c ^B,")
            + "and"
            + equation(
                r"e_{damper}\left(c, \dot c\right) = \frac{A \, L}{B} c^{B-1} \,\dot c,"
            )
