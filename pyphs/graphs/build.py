...
def buildCore(graph):
    ...
    # build J matrix
    graph.analysis.J = vstack(
        hstack(types.matrix_types[0](sympy.zeros(nec)), gamma.T),
        hstack(-gamma, types.matrix_types[0](sympy.zeros(nfc))),
    )
    all_edges = (
        graph.analysis.stor_edges
        + graph.analysis.diss_edges
        + graph.analysis.port_edges
        + graph.analysis.conn_edges
    )
...
def _setCore(graph):
    ...
    for e in graph.analysis.conn_edges:
        ...
        index_e_in_connector = [e_label in c["y"] for c in graph.core.connectors].index(
            True
        )
