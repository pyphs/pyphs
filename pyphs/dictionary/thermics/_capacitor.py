...
class Capacitor(StorageNonLinear):
 ...
 same as the component label:\n{}\nis not \n{}".format(
    ...
    metadata = {
        "title": "Thermal Capacitor",
        "component": "Capacitor",
        "label": "T",
        "dico": "thermics",
        "desc": (
            r"Heat capacity (or mass) with entropy :math:`\sigma\in\mathbb R`, energy (exponential law):"
            + equation(r"H(\sigma)= C\,T_0\,\exp{\left(\frac{\sigma}{C}\right)},")
            + "and temperature:"
            + equation(
                r"\theta(\sigma) = \frac{d H}{d \sigma}(\sigma) = T_0\,\exp{\left(\frac{\sigma}{C}\right)}."
            )
        ),
