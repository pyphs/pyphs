...
class Springcubic(StorageNonLinear):
    def __init__(self, label, nodes, **kwargs):
        ...
        x = symbols("x" + label)
        # storage funcion
        H = F1 * x ** 2 / (2.0 * xref) + F3 * x ** 4 / (4 * xref ** 3)
        N1, N2 = nodes
    ...
    metadata = {
        ...
        "dico": "mechanics",
        "desc": (
            r"Cubic spring with state :math:`q\in \mathbb R` and parameters described below. The energy is"
            + equation(
                r"H(q) = \frac{F_1\,q^2}{2\,q_{ref}} + \frac{F_3\,q^4}{4q_{ref}^3}."
            )
            + "The resulting force is:"
            + equation(
                r"f(q)= \frac{d \, H(q)}{d q} = F_1 \,\frac{q}{q_{ref}} + F_3 \, \frac{q^3}{q_{ref}^3}."
            )
        ...
        "parameters": [
            ["F1", "Linear contribution to restoring force", "N", 1e1],
