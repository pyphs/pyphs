...
def netlist2tex(netlist):
    ...
    with open(os.path.join(path_to_templates, "latex", "netlist.template"), "r") as f:
    ...
    if netlist.nlines() > 0:
        ...
        for comp in netlist:
            ...
            latex_args = r"$\left\{ " + dic2array(comp["arguments"]) + r"\right.$"
            str_table = r"{0} & {1} & {2}.{3} & {4} & {5}".format(
                latex_line, latex_label, latex_dic, latex_comp, latex_nodes, latex_args
            )
...
def graphplot2tex(graph, label=None, folder=None, show=False):
    ...
    with open(os.path.join(path_to_templates, "latex", "graph.template"), "r") as f:
        template = string.Template(f.read())
    subs = {}
