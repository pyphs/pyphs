...
def draw_nodes(graph, ax=None, layout=None, colors=None):
    ...
    nx.draw_networkx_nodes(
        graph, graph.positions, ax=ax, node_size=800, node_color=colors, lw=2
    )
    nx.draw_networkx_labels(graph, graph.positions, ax=ax)


def midle(nodes):
    return (nodes[0] + nodes[1]) / 2.0
...
def draw_edge(edge, pos, ax, move=0.0, colors_type=None, draw=True):
    ...
    bbox_props = dict(
        boxstyle="round, pad=0.3", fc=colors[1], ec=colors[0], lw=2, alpha=1
    )
    v = vector(nodes)
    m = midle(nodes)
    d = direction(v)
...
def plot_analysis(graph, analysis, show=True):
    ...
    ic_edges_labels = [analysis.get_edges_data("label")[i] for i in analysis.ic_edges]

    fc_edges_labels = [analysis.get_edges_data("label")[i] for i in analysis.fc_edges]

    ec_edges_labels = [analysis.get_edges_data("label")[i] for i in analysis.ec_edges]
    ...
    for nodes in multi2single(graph).edges():
        ...
        for e in range(nedges):
            ...
            if not edge[0] == nodes[0]:
                move = -move
            draw_edge(
                edge, graph.positions, ax, move=move, colors_type=color, draw=draw
            )
    plt.tight_layout()
    ax.axes.get_xaxis().set_visible(False)
    ax.axes.get_yaxis().set_visible(False)
