...
class Dissipative(Graph):
    def __init__(self, label, nodes, **kwargs):
        ...
        assert all(
            z_vals[np.nonzero(w_vals >= 0)] >= 0
        ), "All values z(w) for\
 w>=0 must be non-negative (component {})".format(
            label
        )
 ...
 w<0 must be negative (component {})".format(
            label
        )
...
(component {})".format(
        ...
        # init component
        self += edges.DissipativeNonLinear(
            label,
            [
                edge,
            ],
