...
def waveNumbers(N, Lb, Kb, Mb):
    ...
    k = linspace(0.0, nPeriods * 2 * pi / Lb, int(nPoints))
...
def omega_cosine(zp, wp, Lb, k):
    ...
    a = (
        ...
        * (
            cosh(km * Lb)
            * (
                (pi ** 2 + (km * wp) ** 2) * cos((km * wp) / 2) * cos(km * zp)
                - (pi ** 2 - (km * wp) ** 2) * cosh((km * wp) / 2) * cosh(km * zp)
            )
            + (pi ** 2 + (km * wp) ** 2)
            * cos((km * wp) / 2)
            * (cos(km * (Lb - zp)) - sin(km * zp) * sinh(km * Lb))
            - (pi ** 2 - (km * wp) ** 2)
            * cosh((km * wp) / 2)
            * (
                cos(km * Lb) * cosh(km * zp)
                + (sin(km * Lb) - sinh(km * Lb)) * sinh(km * zp)
            )
        )
    )
    b = (pi ** 4 - (km * wp) ** 4) * sqrt(
        -(1 / km)
        * (
            -km * Lb * (2 + cos(2 * km * Lb) + cosh(2 * km * Lb))
            + 2 * cosh(km * Lb) * (-2 * km * Lb * cos(km * Lb) + sin(km * Lb))
            + cosh(km * Lb) ** 2 * sin(2 * km * Lb)
            + 2 * cos(km * Lb) * sinh(km * Lb)
            + (cos(km * Lb) ** 2) * sinh(2 * km * Lb)
        )
    )
    omega = list(a / b)
    return omega
...
class Cantilever(Graph):
    def __init__(self, label, nodes, **parameters):
        ...
        pars.update({"kmodes": k, "fmodes": fk})
        pars["N"] = len(pars["kmodes"])

        # [N/m**1] Modal damping coefficient
        pars.update({"Ab_array": pars["A"] * 10 ** (linspace(0, pars["D"], pars["N"]))})
