...
class Capacitorsat(StorageNonLinear):
    def __init__(self, label, nodes, **kwargs):
        ...
        Hsat = c1 * (sp.log(sp.cos(t1)) + (t1 ** 2) / 2.0)
    ...
    metadata = {
        "title": "Saturating magnetic capacitor",
        "component": "Capacitorsat",
        "label": "capa",
        "dico": "magnetics",
        "desc": (
            r"Saturating magnetic capacity from [1]_ (chap 7) with state :math:`\phi\in [-\phi_{sat}, \phi_{sat}]` and parameters described below. The energy is"
            + equation(
        ...
        "parameters": [
            ["C0", "Magnetic capacitance around zero", "H", 1e3],
